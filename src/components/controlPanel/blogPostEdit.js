...
const BlogPostEdit = () =>{
    ...
    //-----------------------------------------------------------------------source state
     ...
     const [gotTheData , setGotTheData] = useState({});
    ...
    //------------------------------axios------------------------------
    //get blog post edit
        const getTheBlog = async () =>{
            try{
                    const response = await authCtx.jwtInst({
                        method:"get",
                        params:{id:params.blogId , language:langCtx.language},
                        url:`${authCtx.defaultTargetApi}/blog/getTheBlog`,
                        config: { headers: {'Content-Type': 'application/x-www-form-urlencoded' }}
                    })
                    const data = await response.data; 
                    setCoverImg(data.coverImage);
                    setBlogPostTitle(data.title);
                    setBlogPostSubTitle(data.subtitle);
    ...
    useEffect(() => {
        ...
        const updateBlog = async() =>{
            let blogPostData = {author:decoded.id , id:params.blogId ,  content:content  , language:langCtx.language , title:blogPostTitle , subtitle:blogPostSubtitle , source:source , coverImage : coverImg , pageTitle:pageTitle , pageDescription:pageDescription};
            if(blogPostTitle === ''){
            ...
            if(blogPostTitle !== '' && blogPostSubtitle !== '' && decoded.id !== undefined && content !== '' && coverImg !== '' ){
                    try{
                        const response = await authCtx.jwtInst({
                            method:"post",
                            url:`${authCtx.defaultTargetApi}/blog/updateBlog`,
                            data:blogPostData,
                            config: { headers: {'Content-Type': 'application/x-www-form-urlencoded' }}
                        })
                        const data = await response.data; 
                        setSuccessOpenToast(true);
                        setSuccessMsgToast('بلاگ ذخیره شد');
                            ...
                            <div className={Style.inndeDiv}>
                                       ...
                                       <Row>
                                            <Col style={{marginBottom:'14px'}} xs={12} md={12} lg={12 }>
                                                <div ref={ref1}  className={Style.formItemsDiv}>
                                                    ...
                                                    <NormalInput  value={blogPostTitle}  onChange={(e)=>{setBlogPostTitle(e.target.value); setBlogPostTitleErr('')}} placeholder='عنوان را وارد کنید...'></NormalInput>
                                                </div>
                                            </Col>
                                            <Col style={{marginBottom:'14px'}} xs={12} md={12} lg={12 }>
                                                <div ref={ref2}  className={Style.formItemsDiv}>
                                                    ...
                                                    <NormalInput value={blogPostSubtitle} onChange={(e)=>{setBlogPostSubTitle(e.target.value); setBlogPostSubtitleErr('')}} placeholder='توضیحات را وارد کنید...'></NormalInput>
                                                </div>
                                            </Col>
                                       </Row>
                                   </Col>
                               ...
                               <Row ref={ref4} dir="rtl">
                                    <div className={Style.formItemsDiv}>
                                        ...
                                        <Col xs={12} md={12} lg={6}>
                                        {content !==''?
                                            <Ck data={content} ckEditorFunction={ckEditorFunction}></Ck>
                                        :null}
                                        </Col>
                                    </div>
                               </Row>
                               ...
                               <div className={Style.saveBtnDiv}>
                                   <button onClick={updateBlog} className={Style.saveForm}>ذخیره</button>
                               </div>
                            </div>        
                        </Col>
                    </Row>
