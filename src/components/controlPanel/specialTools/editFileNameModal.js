...
const EditFileNameModalPortal =(props)=>{
   ...
   useEffect(() => {
       if(props.showModal ===true){
        setFirstName(props.opratorIdToEdit.metaData.originalname.split("." ,1).pop());
       }
   }, [props.showModal]);
                        ...
                        <div className={props.showModal === true ? `${Style.modalBoarder} ${Style.scaleIn}` : props.showModal === false ? `${Style.modalBoarder} ${Style.scaleOut}` : null} >
                            ...
                            <div className={Style.btnsDiv}>
                                <div className={Style.btnDiv}>
                                    <NormalBtn  onClick={()=>{props.updateOprator(firstName)}} btnName={'ذخیره'} paddingTop={'5px'} paddingButtom={'5px'} fontSize={'20px'} paddingRight={'25px'} paddingLeft={'25px'} backgroundColor={'#1043A9'} color={'#FFFFFF'} ></NormalBtn>
    ...
    return(
        <Fragment>
            {ReactDom.createPortal(
                <EditFileNameModalPortal updateOprator={props.renameTheFile} opratorIdToEdit={props.fileIdToEdit} closeModal={props.closeModal}  showModal={props.showModal} ></EditFileNameModalPortal>
            ,
